	...
	begin
		...
		http.request(request) do |response|
			...
			if code == "302"
				...
				if size != length
					download2(uri, cookies, filename, size)
				elsif
		...
		puts "Error downloading '{filename}' #{$!.message}";
	end
end

def download2(uri, cookies, file_name, offset)
	http = Net::HTTP.start(uri.host)
	request = Net::HTTP::Get.new(uri.request_uri)
	request['cookie'] = cookies
	if offset == nil
		...
		if code == "200" || code == "206"
			...
			begin
				...
				if size != length
					...
					sleep(interval)
					download2(uri, cookies, file_name, size)
				elsif
